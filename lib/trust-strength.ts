...
/** Used when DB `strengthTiersJson` is null or invalid */
...
/** Pick the highest tier whose threshold is still at or below `points`. */
...
/** Map average star rating (1–5) to experience 0–100 */
export function averageRatingToExperience(avg: number): number {
  const scaled = ((avg - 1) / 4) * 100;
  return Math.round(Math.min(100, Math.max(0, scaled)));
}

export function strengthLabelFromPoints(
  points: number,
  strengthTiersJson: Prisma.JsonValue | null | undefined
): string {
  const tiers = parseStrengthTiersJson(strengthTiersJson);
  return resolveStrengthLabel(points, tiers);
}
